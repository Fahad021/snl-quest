...
class BtmCostSavingsReport(WizardReportInterface):
    ...
    def has_chart(self, name):
        ...
        if name in {'demand_charge_comparison_multiset'}:
            return any(op[1].has_demand_charges() for op in self.chart_data)
        elif name in {'energy_charge_comparison_multiset'}:
            return any(op[1].has_energy_charges() for op in self.chart_data)
        elif name in {'nem_comparison_multiset'}:
            return any(op[1].has_nem_charges() for op in self.chart_data)
        else:
            return True
...
class BtmCostSavingsReportScreen(ReportScreen):
    ...
    def generate_total_bill_comparison_chart(self, *args):
        ...
        colors = PALETTE if n_cats > len(PALETTE) else sample(PALETTE, n_cats)
    ...
    def generate_energy_charge_comparison_chart(self, *args):
        ...
        if any(op[1].has_energy_charges() for op in self.chart_data):
            ...
            report_templates = [total_difference_str]
            self.desc.text += ' '.join(report_templates)
            self.is_drawn = True
        else:
    ...
    def generate_peak_demand_comparison_chart(self, *args):
        ...
        self.desc.text = (
            'The peak demand value each month is used to compute flat demand charges, if applicable. '
    ...
    def generate_total_savings_donut_chart(self, *args):
        ...
        if (
            sum(x[1].nem_charge_without_es for x in self.chart_data)
            + sum(x[1].nem_charge_with_es for x in self.chart_data)
            <= 0
        ):
            n_cats = 2
        else:
...
class BtmCostSavingsGenerateReportMenu(ModalView):
    ...
    def generate_report_screens(self):
        ...
        for ix, opt in enumerate(self.host_report.chart_types.items(), start=0):
            chart_name = opt[1]
            if has_chart_status := self.host_report.has_chart(chart_name):
    ...
    def generate_executive_summary(self):
        ...
        demand_charge_strings = [
            "Without energy storage, the peak demand observed during the evaluation period was <b>{peak_demand_without_es:.2f} kW</b>. By adding energy storage, this value was changed to <b>{peak_demand_with_es:.2f} kW</b>.".format(
        ...
        if any(op[1].has_demand_charges() for op in chart_data):
            ...
            relation = 'decrease' if total_demand_charge_difference < 0 else 'increase'
        ...
        executive_summary_strings = [
            total_bill_summary,
            bill_comparison_summary,
            demand_charge_summary,
            ' '.join(demand_charge_strings),
        ]
    ...
    def generate_report_from_template(self):
        ...
        for chart_caption, chart_type in chart_types.items():
            if self.host_report.has_chart(chart_type):
                chart_info = {
                    'path': self.graphics_locations[chart_type],
                    'ix': chart_ix,
