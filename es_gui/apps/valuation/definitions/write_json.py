...
nodes = {
    iso: {
        row[0]: {'name': row[1], 'ID': row[0]}
        for row in zip(
            node_sheets[iso]['Node ID'], node_sheets[iso]['Node Name']
        )
    }
...
for iso in IMPLEMENTED_ISO:
    ...
    if iso in {'PJM', 'MISO', 'ERCOT', 'ISO-NE'}:
        tmp_list.extend(
            (
                {
                    'name': 'frac. of reg. up capacity deployed',
                    'attr name': 'fraction_reg_up',
                    'default': 0.25,
                },
                {
                    'name': 'frac. of reg. down capacity deployed',
