...
def download_utdata(**kwargs):
    ...
    des_file_iou = f"{des_dir}iouzipcodes2017.csv"
    des_file_noniou = f"{des_dir}noniouzipcodes2017.csv"
    ...
    if succeed:
        ...
        return pd.concat([df_iou,df_noniou],ignore_index=True)
    else:
        return pd.DataFrame()
...
def search_utdata_byname(**kwargs):
    ...
    if len(utdataframe)>0:
        ...
        return utdatabyname.drop_duplicates()
    else:
        return pd.Dataframe()
...
def search_utdata_byzip(**kwargs):
    ...
    if len(utdataframe)>0:
        ...
        return utdatabyzip.drop_duplicates()
    else:
        return pd.Dataframe()
...
def search_utdata_bystate(**kwargs):
    ...
    if len(utdataframe)>0:
        ...
        return utdatabystate.drop_duplicates()
    else:
        return pd.Dataframe()
...
def find_utschld(**kwargs):
    ...
    des_file = f"{des_dir}schedule.json"
...
def read_load_profile(path, month):
    ...
    return load_df_month[data_column_name].values
...
def input_df(year,wkday_eschld,wkend_eschld,wkday_dschld,wkend_dschld):
                ...
                if (date in us_holidays) or (dayofweek>4):
                    row = (
                        [
                            ...
                            day,
                            hour,
                            wkend_eschld[month - 1][hour - 1],
                            wkend_dschld[month - 1][hour - 1],
                        ]
    ...
    return pd.DataFrame(data=data_list,columns=column_list)
